	...
	public class TonemappingColorGrading : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (tonemapping.enabled)
			{
				...
				{
				case Tonemapper.Curve:
					if (m_TonemapperDirty)
					{
						float num15 = 1f;
						if (tonemapping.curve.length > 0)
						{
							num15 = tonemapping.curve[tonemapping.curve.length - 1].time;
							for (float num16 = 0f; num16 <= 1f; num16 += 0.003921569f)
							{
								float num17 = tonemapping.curve.Evaluate(num16 * num15);
								tonemapperCurve.SetPixel(Mathf.FloorToInt(num16 * 255f), 0, new Color(num17, num17, num17));
							}
							tonemapperCurve.Apply();
						}
						m_TonemapperCurveRange = 1f / num15;
				...
				{
					float num11 = tonemapping.neutralBlackIn * 20f + 1f;
					float num12 = tonemapping.neutralBlackOut * 10f + 1f;
					float num13 = tonemapping.neutralWhiteIn / 20f;
					float num14 = 1f - tonemapping.neutralWhiteOut / 20f;
					float t = num11 / num12;
					float t2 = num13 / num14;
