...
namespace CommandSystem.Commands.RemoteAdmin
{
	...
	public class ElevatorTeleportCommand : ICommand
	{
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			if (!ElevatorDoor.AllElevatorDoors.TryGetValue(group, out var value) || value.Count == 0)
			{
			...
			Vector3 position = value[0].TargetPanel.AssignedChamber.transform.position + Vector3.up * PositionOffset;
			string text = "inside";
			if (newargs != null && newargs.Length > 0)
			{
				...
				case "out":
				case "outside":
					position = DoorTPCommand.EnsurePositionSafety(value[0].TargetPanel.AssignedChamber.CurrentDestination.transform);
					text = "outside";
					break;
				default:
				{
					...
					if (result < 0 || result >= value.Count)
					{
						response = "Selected elevator doesn't have level " + newargs[0] + ".";
						return false;
					}
					position = DoorTPCommand.EnsurePositionSafety(value[result].transform);
					text = $"level {result}";
					break;
				}
				case "i":
			...
			{
				if (item.TryOverridePosition(position, Vector3.zero))
				{
					if (num != 0)
					{
