...
namespace CommandSystem.Commands.RemoteAdmin
{
	...
	public class ElevatorCommand : ParentCommand, IUsageProvider
	{
		...
		internal static bool TryParseGroup(string txt, out ElevatorManager.ElevatorGroup group)
		{
			if (!int.TryParse(txt, out var result))
			{
				return Enum.TryParse<ElevatorManager.ElevatorGroup>(txt, ignoreCase: true, out group);
			}
			group = (ElevatorManager.ElevatorGroup)result;
			return Enum.IsDefined(typeof(ElevatorManager.ElevatorGroup), group);
		}
	}
}
