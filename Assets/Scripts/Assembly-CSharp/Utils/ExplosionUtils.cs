...
namespace Utils
{
    public static class ExplosionUtils
    {
        ...
        {
            ServerExplode(hub.transform.position, new Footprint(hub));
        }

        public static void ServerExplode(Vector3 position, Footprint footprint)
        {
            if (InventoryItemLoader.TryGetItem<ThrowableItem>(ItemType.GrenadeHE, out var result) && result.Projectile is ExplosionGrenade settingsReference)
            {
                ...
                ExplosionGrenade.Explode(footprint, position, settingsReference);
            }
        }
