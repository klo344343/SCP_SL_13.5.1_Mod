...
namespace FacilitySoundtrack
{
    public class AloneSoundtrack : SoundtrackLayerBase
    {
        ...
        {
            if (ReferenceHub.TryGetLocalHub(out var hub) && (hub.GetTeam() == Team.SCPs || !hub.IsAlive() || hub.GetRoleId() == RoleTypeId.Tutorial))
            {
                _weight = 0f;
                SetVolume(_weight);
                return;
            ...
            {
                if (!allHub.isLocalPlayer && allHub.IsAlive() && allHub.GetRoleId() != RoleTypeId.Tutorial && VisionInformation.GetVisionInformation(hub, hub.PlayerCameraReference, allHub.PlayerCameraReference.position, 0.25f, 60f).IsLooking)
                {
                    if (!allHub.IsSCP())
                    {
                        flag = true;
            ...
            float num = ((IsMuted || _oldAloneTime != 0f || 120f > _aloneTime) ? (0f - _fadeOutSpeed) : _fadeInSpeed);
