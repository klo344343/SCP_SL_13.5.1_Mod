    ...
    public class DamagedAudio : MonoBehaviour
    {
        ...
        private void PlayAudio(float damage, bool isHume)
        {
            if ((_owner.isLocalPlayer || _owner.IsLocallySpectated()) && !(damage <= 2.5f) && !(_lastTime > NetworkTime.time))
            {
                ...
                AudioSourcePoolManager.PlaySound(isHume ? _scpAudioClips[Random.Range(0, _scpAudioClips.Length)] : _humanAudioClips[Random.Range(0, _humanAudioClips.Length)], _owner.transform, 5f, 1f, FalloffType.Exponential, AudioMixerChannelType.DefaultSfx, 0f);
            }
        }
    }
}
