...
public class ServerConsole : MonoBehaviour, IDisposable
{
    ...
    public static List<IOutput> ConsoleOutputs;
    ...
    {
        _serverName = ConfigFile.ServerConfig.GetString("server_name", "My Server Name");
    }
    ...
    {
        CheckRoot();
        ConsoleOutputs = new List<IOutput>();
        NameFormatter = new InterpolatedCommandFormatter
        {
            ...
            Commands = new Dictionary<string, Func<List<string>, string>>
            {
                ...
                {
                    "player_count",
                    (List<string> args) => ReferenceHub.AllHubs.Count((ReferenceHub hub) => hub.Mode == ClientInstanceMode.ReadyClient).ToString()
                },
                {
                    "full_player_count",
                    delegate(List<string> args)
                    {
                        int num = ReferenceHub.AllHubs.Count((ReferenceHub hub) => hub.Mode == ClientInstanceMode.ReadyClient);
                        return (num == CustomNetworkManager.TypedSingleton.ReservedMaxPlayers) ? (args.Count switch
                        {
                            1 => "FULL",
                        ...
                        }) : $"{num}/{CustomNetworkManager.TypedSingleton.ReservedMaxPlayers}";
                    }
                },
                {
                    "max_players",
                ...
                {
                    "mtf_respawns",
                    (List<string> args) => GetRoundInfo((RoundSummary summary) => (UnitNameMessageHandler.ReceivedNames.TryGetValue(SpawnableTeamType.NineTailedFox, out var value) ? (value.Count - 1) : 0).ToString(), intValue: true)
                },
                {
                    "warhead_detonated",
        ...
        ReferenceHub.OnPlayerRemoved = (Action<ReferenceHub>)Delegate.Combine(ReferenceHub.OnPlayerRemoved, (Action<ReferenceHub>)delegate
        {
    ...
    public static void AddLog(string q, ConsoleColor color = ConsoleColor.Gray)
    {
        ...
        while (!_disposing)
        {
            ...
            else
            {
                ...
                if (!AuthenticatorQuery.SendData(list2) && !_printedNotVerifiedMessage)
                {
                    ...
                    {
                        AddLog("If you are 100% sure that the server is working, can be accessed from the Internet and YOU WANT TO MAKE IT PUBLIC please email following information:", ConsoleColor.Red);
                        AddLog("- IP address of server (most likely " + Ip + ")", ConsoleColor.Red);
                        AddLog("- port of the server (currently the server is running on port " + PortToReport + ")", ConsoleColor.Red);
                        AddLog("- is this static or dynamic IP address (most of home adresses are dynamic)", ConsoleColor.Red);
                        AddLog("PLEASE READ rules for verified servers first: https://scpslgame.com/Verified_server_rules.pdf", ConsoleColor.Red);
                        AddLog("send us that information to: server.verification@scpslgame.com (server.verification at scpslgame.com)", ConsoleColor.Red);
                        AddLog("if you can't see the AT sign in console (in above line): server.verification AT scpslgame.com", ConsoleColor.Red);
                        AddLog("email must be sent from email address set as \"contact_email\" in your config file (current value: " + ConfigFile.ServerConfig.GetString("contact_email") + ").", ConsoleColor.Red);
                    }
                }
                else
                {
