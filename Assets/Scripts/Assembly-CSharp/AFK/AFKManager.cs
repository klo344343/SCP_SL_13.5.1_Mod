    ...
    public static class AFKManager
    {
        ...
        private static void ConfigReloaded()
        {
            ...
            if (_kickTime <= 0f)
            {
                ...
                {
                    _eventsStatus = false;
                    ReferenceHub.OnPlayerAdded = (Action<ReferenceHub>)Delegate.Remove(ReferenceHub.OnPlayerAdded, new Action<ReferenceHub>(AddPlayer));
                    ReferenceHub.OnPlayerRemoved = (Action<ReferenceHub>)Delegate.Remove(ReferenceHub.OnPlayerRemoved, new Action<ReferenceHub>(RemovePlayer));
            ...
            {
                _eventsStatus = true;
                ReferenceHub.OnPlayerAdded = (Action<ReferenceHub>)Delegate.Combine(ReferenceHub.OnPlayerAdded, new Action<ReferenceHub>(AddPlayer));
                ReferenceHub.OnPlayerRemoved = (Action<ReferenceHub>)Delegate.Combine(ReferenceHub.OnPlayerRemoved, new Action<ReferenceHub>(RemovePlayer));
                StaticUnityMethods.OnUpdate += OnUpdate;
                PlayerRoleManager.OnRoleChanged += RoleChange;
            }
        }
