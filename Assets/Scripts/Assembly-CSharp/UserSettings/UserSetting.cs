...
namespace UserSettings
{
    public static class UserSetting<T>
    {
        ...
        public static void Save<TEnum>(TEnum key, T value) where TEnum : Enum
        {
            ushort typeHash = SettingsKeyGenerator.GetStableTypeHash(typeof(TEnum));
            ushort numValue = (ushort)Convert.ToInt32(key);
            string prefsKey = SettingsKeyGenerator.TypeValueToKey(typeHash, numValue);
            GetHandler().Save(prefsKey, value);
            NotifyListeners(typeHash, numValue, value);
        }

        public static T Load(string key, T fallback = default)
        {
            return GetHandler().Load(key, fallback);
        }

        public static void Save(string key, T value)
        {
            GetHandler().Save(key, value);
        }

        private static void NotifyListeners(ushort typeHash, ushort numValue, T value)
        {
            foreach (var listener in Listeners)
            {
                if (listener.TypeHash == typeHash && listener.NumericalValue == numValue)
                {
                    listener.Event?.Invoke(value);
                }
            }
        }
        ...
        public static T GetDefaultValue(ushort type, ushort val)
        {
